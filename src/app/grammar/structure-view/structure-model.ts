...
export function buildStructure(
  ...
  for (const file of [...unit.files, ...extraFiles]) {
    ...
    if (included && !isMain) {
      edges.push({ id: `e:files:${file.path}`, source: CONFIG_ID, target: fileId, kind: 'files' });
    }
    ...
    for (const section of file.sections) {
      ...
      if (declared && section.kind !== 'CONFIG') {
        edges.push({ id: `e:declares:${sectionId}`, source: CONFIG_ID, target: sectionId, kind: 'declares' });
      }
